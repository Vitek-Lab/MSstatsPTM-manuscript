...
summary_df$PTM$ProcessedData
summary_df$PTM$ProcessedData$global_prot = sapply(summary_df$PTM$ProcessedData$PROTEIN, function(x){str_split(x, "_")[[1]][1]})

summary_df$PTM$ProcessedData$global_prot %>% n_distinct()
summary_df$PTM$ProcessedData$PROTEIN %>% n_distinct()
summary_df$PTM$ProcessedData$FEATURE %>% n_distinct()

summary_df$PROTEIN$ProcessedData$PROTEIN %>% n_distinct()
summary_df$PROTEIN$ProcessedData$PEPTIDE %>% n_distinct()

summary_df$PTM$RunlevelData
...
# Volcano plot
png("../supplementary/sim_new/spike_in_msstatsptm_volcano.png", width = 750, height = 500)
...
# Volcano plot
png("../supplementary/sim_new/spike_in_limma_volcano.png", width = 750, height = 500)
plot_df %>% arrange(labeling) %>% 
  filter(abs(Log2FC) < Inf & Label %in% c("mix1-mix3", "mix1-mix4", "mix2-mix3", "mix2-mix4")) %>%
  ggplot(aes(Log2FC, -log10(adj.pvalue))) + 
  geom_point(aes(color = labeling, alpha = labeling), size = 2.2) + 
