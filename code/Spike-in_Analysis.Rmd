---
title: "Spike-in_MSstatsPTM_Analysis"
author: "Devon Kohler"
date: "6/18/2021"
...
load(file = "model_data/Spike_in_MSstatsPTM_Model.rda")#model_df
...
load(file = "model_data/Spike_in_Limma_Model.rda")#limma_model
...
```{r, echo=FALSE, fig.width = 10, fig.height = 7}
load(file = "model_data/Spike_in_ttest_model.rda")#ttest_final_model
...
summary %>% mutate(Model = factor(Model, levels=c("Proposed", "Limma", "T-test"))) %>% 
  ...
  labs(title = "Summary Statistic Comparison", x = "Metric", y = "Result") + theme_bw()
dev.off()
```
...
unadj_spike_ttest <- ttest_final_model %>% filter(labeling == "heavy" & protadj == "no adjustment") %>% rename(log2FC = estimate)
unadj_spike_ttest$Adjustment <- "no adjustment"
unadj_spike_ttest$Model <- "T-test"
adj_spike_ttest <- ttest_final_model %>% filter(labeling == "heavy" & protadj != "no adjustment") %>% rename(log2FC = estimate)
adj_spike_ttest$Adjustment <- "protein adjustment"
adj_spike_ttest$Model <- "T-test"

unadj_spike_limma <- limma_model$PTM.Model %>% filter(labeling == "heavy") %>% rename(log2FC = Log2FC)
unadj_spike_limma$Adjustment <- "no adjustment"
unadj_spike_limma$Model <- "Limma"
adj_spike_limma <- limma_model$ADJUSTED.Model %>% filter(labeling == "heavy") %>% rename(log2FC = Log2FC)
...
spike[Model == "MSstatsPTM"]$Model <- "Proposed"
...
spike %>% mutate(Comparison = str_split(contrast_truth, " ", simplify = TRUE)[,1],
                 Model = factor(Model, levels=c("Proposed", "Limma", "T-test"))) %>%  ggplot() + 
  geom_boxplot(aes(x = Comparison, y = log2FC, fill = Model)) + 
  geom_point(aes(y = trueLog2FC, x = Comparison), color = "red", shape = 4, size = 4, stroke = 2) + 
  scale_fill_manual(values = c("salmon1", "steelblue1", "green")) + facet_wrap(.~Adjustment) + 
    theme_bw() + 
