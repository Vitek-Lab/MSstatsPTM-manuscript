...
spike %>% mutate(Comparison = str_split(contrast_truth, " ", simplify = TRUE)[,1],
      Model = factor(Model, levels=c("Proposed", "Limma", "Anova"))) %>%
...
setDT(spike)[,list(IQR=IQR(log2FC, na.rm = TRUE)),by=list(Model, Label, protadj)] %>% 
  filter(protadj == 'protein adjustment') %>% 
  group_by(Model) %>% summarize(mean(IQR))

spike_se = setDT(spike)[,list(SE=mean(SE, na.rm = TRUE)),by=list(Model, Label, protadj)] %>% 
  filter(protadj == 'protein adjustment') %>% 
  group_by(Model) %>% summarize(mean(SE))

all_se = setDT(spike)[,list(SE=mean(SE, na.rm = TRUE)),by=list(Model, Label, protadj)] %>% 
  filter(protadj == 'protein adjustment') %>% 
  group_by(Model) %>% summarize(mean(SE))

(0.5984246 - 0.9309134) / 0.9309134
