...
calc_roc <- function(df, protein_col_name, pval_col_name){
  
  temp_df <- df %>% select(get(protein_col_name), get(pval_col_name))
  
  temp_df$True <- ifelse(grepl("NoChange", get(protein_col_name)), 0, 1)
  temp_df$Pred <- ifelse(get(pval_col_name) < .05, 1, 0)
...
ttest_results$Color_Model <- "T-Test"
ttest_results$Adj = FALSE
ttest_adj_results$Model <- "T-Test"
ttest_adj_results$Color_Model <- "T-Test"
ttest_adj_results$Adj = TRUE
...
png("../main/images/sim1_FDR_all_models.png", width = 750, height = 500)
models %>% mutate(Model = factor(Model, levels=c("Proposed", "Proposed no adjustment", "Limma", 
  ...
  scale_fill_manual(values=cbPalette_first) +
...
load(file = "../data/ptm_models_sim2.rda")#ptm_models_sim1
load(file = "../data/adjusted_models_sim2.rda")#adjusted_models_sim1 
load(file = "../data/ttest_models_sim2.rda")#ttest_sim1
load(file = "../data/adj_ttest_models_sim2.rda")#adj_ttest_sim1
load(file = "../data/limma_models_sim2.rda")#limma_results_sim1
load(file = "../data/adj_limma_models_sim2.rda")#adj_limma_sim1
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
png("../main/images/sim3_Accuracy.png", width = 750, height = 500)
models %>% mutate(Model = factor(Model, levels=c("Proposed", "Proposed_no_adj", "limma_no_adj", 
                                               "Limma", "ttest_no_adj", "T-Test")),
                  Replicates = as.factor(rep)) %>%
  filter(Model %in% c("Proposed", "T-Test", "Limma")) %>% 
  ...
  theme(axis.text.x = element_text(angle = 60, hjust=1, size = 16), 
         ...
         title = element_text(size = 22),
         strip.text = element_text(size = 16)) + 
  labs(title = "Simulation 2: Model Accuracy", x = "Model", y = "Accuracy") + 
   guides(color = guide_legend(order = 1, override.aes = list(size=5)), 
              size = guide_legend(order = 2, reverse = TRUE))
dev.off()
```
...
## Caluclate #s of IQR
setDT(boxplot_data)[,list(IQR=IQR(log2FC, na.rm = TRUE)),by=list(Model)]

(0.3787906 - 0.4228233) / 0.4228233
(0.3787906 - 0.4843184) / 0.4843184
