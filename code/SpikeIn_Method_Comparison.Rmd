---
title: "SpikeIn MSstatsPTM Method Comparison"
author: "Devon Kohler"
date: "3/7/2022"
output:
...
calculate_summary_stats <- function(df, pval_col_name, comparison_col_name){
  ...
  fpr <- df %>% filter((get(pval_col_name) < .05 & labeling == "light") | 
                (get(comparison_col_name) == "mix1-mix4" & get(pval_col_name) < .05)) %>% 
    nrow() / nrow(df %>% filter(labeling == "light"))
  
  ## TP/P (TPR)
  recall <- df %>% filter(get(pval_col_name) < .05 & labeling == "heavy" & 
                                 get(comparison_col_name) != "mix1-mix4") %>% 
    nrow() / df %>% filter(labeling == "heavy" & 
                             get(comparison_col_name) != "mix1-mix4") %>% nrow()
  
  ## TN/N (TNR)
  specificity <- df %>% filter(get(pval_col_name) >= .05 & (labeling == "light" |
                                 get(comparison_col_name) == "mix1-mix4")) %>% 
    nrow() / nrow(df %>% filter(labeling == "light" | get(comparison_col_name) == "mix1-mix4"))
...
load(file = "MSstatsPTM_Summarized.rda")
load(file = "Spike_in_MSstatsPTM_Model.rda")#model_df
...
plot_df[plot_df$Label == 'mix1-mix3'][['contrast_truth']] <- "LogFC: 1 (mix1-mix3)"
plot_df[plot_df$Label == 'mix1-mix4'][['contrast_truth']] <- "LogFC: 0 (mix1-mix4)"
plot_df[plot_df$Label == 'mix2-mix3'][['contrast_truth']] <- "LogFC: -2 (mix2-mix3)"
plot_df[plot_df$Label == 'mix2-mix4'][['contrast_truth']] <- "LogFC: -1 (mix2-mix4)"

plot_df[plot_df$labeling == 'light'][['labeling']] <- "Background Peptides"
plot_df[plot_df$labeling == 'heavy'][['labeling']] <- "SpikeIn Peptides"
...
spike %>% filter(!Label %in% c("mix1-mix2", "mix3-mix4")) %>% 
  ggplot() + geom_boxplot(aes(x = contrast_truth, y = log2FC), fill = "lightsteelblue4") + 
...
load(file = "Spike_in_Limma_Model.rda")#limma_model
...
combo_plot_df %>% arrange(Model, labeling) %>% 
  filter(abs(log2FC) < Inf & Label %in% c("mix2-mix3", "mix2-mix4")) %>%
  ggplot(aes(log2FC, -log10(adj.pvalue))) + 
  geom_point(aes(color = labeling, alpha = labeling), size = 2.2) + 
  scale_color_manual(values = c("#999999", "#D55E00")) + 
  scale_alpha_manual(values = c(0.2, 1)) + 
...
```{r, echo=FALSE, fig.width = 10, fig.height = 7}
load(file = "Spike_in_ttest_model.rda")#ttest_final_model

## Update some naming conventions
ttest_final_model[ttest_final_model$label == 'mix1-mix3'][['contrast_truth']] <- "LogFC: 1 (mix1-mix3)"
ttest_final_model[ttest_final_model$label == 'mix1-mix4'][['contrast_truth']] <- "LogFC: 0 (mix1-mix4)"
ttest_final_model[ttest_final_model$label == 'mix2-mix3'][['contrast_truth']] <- "LogFC: -2 (mix2-mix3)"
ttest_final_model[ttest_final_model$label == 'mix2-mix4'][['contrast_truth']] <- "LogFC: -1 (mix2-mix4)"
...
```{r, echo = FALSE, fig.width = 10, fig.height = 5}
unadj_spike <- ttest_final_model %>% filter(protadj == "no adjustment" & labeling == "SpikeIn Peptides")
unadj_spike$Adjustment <- "no adjustment"
adj_spike <- ttest_final_model %>% filter(protadj == "protein adjustment" & labeling == "SpikeIn Peptides")
adj_spike$Adjustment <- "protein adjustment"
spike <- rbindlist(list(unadj_spike, adj_spike), fill = TRUE)

spike %>% 
  filter(!label %in% c("mix1-mix2", "mix3-mix4")) %>% 
  ggplot() + geom_boxplot(aes(x = contrast_truth, y = estimate), fill = "lightsteelblue4") + 
  geom_point(aes(y = trueLog2FC, x = contrast_truth), color = "red", shape = 4, size = 3) + facet_wrap(.~Adjustment) + 
...
ttest_sum <- calculate_summary_stats(ttest_final_model %>% filter(protadj == "protein adjustment"), 'adj.pvalue', "label")

# ttest_sum <- data.frame(metric = c('FPR', 'Recall','Specificity','Precision','Accuracy'),
#            result = c(0,0,0,0,0))
ttest_sum$Adjustment <- "protein adjustment"
ttest_sum$Model <- "T-test"        

summary <- rbindlist(list(msstatsptm_sum, limma_sum, ttest_sum))
summary[Model == "MSstatsPTM"]$Model <- "Proposed"

summary %>% mutate(Model = factor(Model, levels=c("Proposed", "Limma", "T-test"))) %>% 
