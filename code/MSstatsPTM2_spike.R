...
fn_kgg <- paste0("vistaga_", design_kgg$run_id, "_kgg_with_unmodified_peptides.tsv")
...
aqua1 <- bind_rows(aqua1_1, aqua1_2) %>% 
  select(PeptideSequence = Label, run_id = `Run ID`, ann_label = `Run Label`, Intensity = Area)
...
rm(list = ls()[!(ls() %in% c("aqua", "design_prot", "design_kgg", "dta_prot", "dta_kgg"))])
...
design_prot <- design_prot %>% 
  ...
  mutate(TechReplicate = str_c("T", id_injectionset), Run = str_c(group, TechReplicate, sep = "-"),
...
df_prot <- dta_prot %>% 
  ...
  mutate(PeptideSequence = str_extract(peptide_ext, "(?<=\\.)[ACDEFGHIKLMNPQRSTVWY\\*]+(?=\\.)")) %>% 
  mutate(
...
## Rename columns
colnames(df_site) <- c('PeptideSequence', 'PrecursorCharge', 'is_mod', 'PSM', 'Condition', 
                       'Run', 'BioReplicate', 'Intensity', 'FragmentIon', 
                       'ProductCharge', 'IsotopeLabelType', 'ProteinName')
colnames(df_prot) <- c('ProteinName', 'PeptideSequence', 'PrecursorCharge', 'is_mod',
                       'PSM', 'Condition', 'Run', 'BioReplicate', 'Intensity',
...
df_prot %>% 
  semi_join(df_fasta %>% rename(ProteinName = uniprot_iso) %>% filter(str_detect(entry_name, "HUMAN"))) %>% 
  ggplot(aes(Run, log2(Intensity))) + 
  geom_boxplot() + 
  geom_vline(xintercept = c(2.5, 4.5, 6.5), linetype = "dashed") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(x = "Run", y = "Log2-intensity", title = "Global profiling - human proteins")
...
aqua_seq <- readxl::read_xls("parkincancerKGG50mix_Masses.xls") %>% 
  ...
  mutate(peptide_annot = str_to_upper(peptide) %>% str_replace("\\(GG\\)", "\\*"))

# [TODO]: confirm EF1A1+
aqua_seq <- aqua_seq %>% 
  mutate(entry_name = ifelse(entry_name == "EF1A1+_HUMAN", "EF1A1_HUMAN", entry_name)) %>% 
  left_join(df_fasta %>% select(uniprot_ac, uniprot_iso, entry_name)) %>% 
  filter(uniprot_ac == uniprot_iso)

aqua_site <- locatePTM(
  aqua_seq$peptide_annot, aqua_seq$uniprot_iso, df_fasta, mod_residue, mod_symbol
) %>% 
...
aqua_summarized <- aqua %>% 
  ...
  select(protein = uniprot_iso, site, PeptideSequence, group, TechReplicate, BioReplicate, Run, Intensity)

# aqua_summarized$site <- str_replace(aqua_summarized$site, ' ', '')
# aqua_summarized$Protein <- as.factor(paste(aqua_summarized$protein, aqua_summarized$site, sep = '_'))
# aqua_summarized$LogIntensities <- log2(aqua_summarized$Intensity)
# aqua_summarized$NumMeasuredFeature <- 0
# aqua_summarized$MissingPercentage <- 0
# aqua_summarized$more50missing <- FALSE
# aqua_summarized$NumImputedFeature <- 0
# aqua_summarized$originalRUN <- as.factor(paste0(aqua_summarized$group, 
#                                       aqua_summarized$TechReplicate, sep = "_"))
# aqua_summarized$GROUP <- as.factor(as.numeric(factor(aqua_summarized$group, levels=unique(aqua_summarized$group))))
# aqua_summarized$RUN <- as.factor(as.numeric(factor(aqua_summarized$Run, levels=unique(aqua_summarized$Run))))
# aqua_summarized$GROUP_ORIGINAL <- as.factor(aqua_summarized$group)
# aqua_summarized$SUBJECT_ORIGINAL <- as.factor(aqua_summarized$BioReplicate)
# aqua_summarized$SUBJECT <- as.factor(as.numeric(factor(aqua_summarized$BioReplicate, 
#                                              levels=unique(aqua_summarized$BioReplicate))))
# aqua_summarized$SUBJECT_NESTED <- as.factor(paste(aqua_summarized$GROUP, aqua_summarized$SUBJECT, sep = "."))
# 
# aqua_summarized <- aqua_summarized %>% select(RUN, Protein, LogIntensities, LogIntensities,
#                                               NumMeasuredFeature, MissingPercentage, more50missing,
#                                               NumImputedFeature, originalRUN, GROUP,
...
#aqua_summarized$Fraction <- 1
aqua_summarized$PSM <- paste(aqua_summarized$PeptideSequence, aqua_summarized$PrecursorCharge, sep = '_')
#aqua_summarized$feature <- paste(aqua_summarized$PeptideSequence, aqua_summarized$PrecursorCharge,
#                                 aqua_summarized$FragmentIon, aqua_summarized$ProductCharge, sep = '_')
aqua_summarized$Condition <- aqua_summarized$group
aqua_summarized$ProteinName <- paste(aqua_summarized$protein, aqua_summarized$site, sep = "_")
...
aqua_summarized <- aqua_summarized %>% left_join(df_site %>% distinct(BioReplicate, Run), by = "BioReplicate")
## Summarize and model ---------------------------------------------------------
...
# aqua_summary_df <- dataSummarizationPTM(aqua_df, normalization.PTM = "equalizeMedians",
#                                         MBimpute = FALSE, 
#                                         MBimpute.PTM = FALSE)
temp_summary_df <- dataSummarizationPTM(input_df, normalization.PTM = "equalizeMedians",
                                   MBimpute = FALSE,
                                   MBimpute.PTM = FALSE)
...
save(model_df, file = 'MSstatsPTM_model.rda')

model_df$PTM.Model <- model_df$PTM.Model %>% mutate(Label = ifelse(Label == "mix2 vs mix1", "mix1-mix2", 
...
model_df$ADJUSTED.Model <- model_df$ADJUSTED.Model %>% mutate(Label = ifelse(Label == "mix2 vs mix1", "mix1-mix2", 
                                                                   ifelse(Label == "mix3 vs mix1", "mix1-mix3",
                                                                          ifelse(Label == "mix4 vs mix1", "mix1-mix4",
                                                                                 ifelse(Label == "mix2 vs mix3", "mix2-mix3",
                                                                                        ifelse(Label == "mix2 vs mix4", "mix2-mix4",
                                                                                               "mix3-mix4"))))))
model_df$PROTEIN.Model <- model_df$PROTEIN.Model %>% mutate(Label = ifelse(Label == "mix2 vs mix1", "mix1-mix2", 
                                                                   ifelse(Label == "mix3 vs mix1", "mix1-mix3",
                                                                          ifelse(Label == "mix4 vs mix1", "mix1-mix4",
                                                                                 ifelse(Label == "mix2 vs mix3", "mix2-mix3",
                                                                                        ifelse(Label == "mix2 vs mix4", "mix2-mix4",
...
summarized_ptm <- summarized_ptm %>% mutate(Abundance = ifelse(str_detect(Run, "mix3|mix4"), 
...
limma_test_res$limma_adj_test <- limma_test_res$limma_adj_test %>% mutate(Label = ifelse(Label == "classmix_1-classmix_2", "mix1-mix2", 
...
limma_test_res$limma_test <- limma_test_res$limma_test %>% mutate(Label = ifelse(Label == "classmix_1-classmix_2", "mix1-mix2", 
                                                                             ifelse(Label == "classmix_1-classmix_3", "mix1-mix3",
                                                                                    ifelse(Label == "classmix_1-classmix_4", "mix1-mix4",
                                                                                           ifelse(Label == "classmix_2-classmix_3", "mix2-mix3",
                                                                                                  ifelse(Label == "classmix_2-classmix_4", "mix2-mix4",
...
ttest_model$ttest_adj_temp <- ttest_model$ttest_adj_temp %>% mutate(label = ifelse(label == "mix1 vs mix2", "mix1-mix2", 
                                                                             ifelse(label == "mix1 vs mix3", "mix1-mix3",
                                                                                    ifelse(label == "mix1 vs mix4", "mix1-mix4",
                                                                                           ifelse(label == "mix2 vs mix3", "mix2-mix3",
                                                                                                  ifelse(label == "mix2 vs mix4", "mix2-mix4",
                                                                                                         "mix3-mix4"))))))
ttest_model$ttest_temp <- ttest_model$ttest_temp %>% mutate(label = ifelse(label == "mix1 vs mix2", "mix1-mix2", 
                                                                           ifelse(label == "mix1 vs mix3", "mix1-mix3",
                                                                                  ifelse(label == "mix1 vs mix4", "mix1-mix4",
                                                                                         ifelse(label == "mix2 vs mix3", "mix2-mix3",
                                                                                                ifelse(label == "mix2 vs mix4", "mix2-mix4",
...
# ## Fix model differences
#ttest_model$ttest_adj_temp[label == "mix1-mix2"]$estimate <- ttest_model$ttest_adj_temp[label == "mix1-mix2"]$estimate*-1
...
#ttest_model$ttest_temp[label == "mix1-mix2"]$estimate <- ttest_model$ttest_temp[label == "mix1-mix2"]$estimate*-1
