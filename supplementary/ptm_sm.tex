...
\centering
 \begin{subfigure}{\textwidth}
  \centering
	...
	\caption{All the considered methods in simulation 1 correctly calibrated FDR  when adjusting for changes in protein abundance. In comparison, the methods without accounting for the protein-level changes resulted in off-target, high false discovery rates.}
 \end{subfigure}
 \begin{subfigure}{\textwidth}
  \centering
...
\caption{Using the proposed method to model the benchmark experiment, the spike in peptides (colored red) do not follow the expected log fold change before adjustment. After adjusting for changes in overall protein abundance the spike in peptides are more in line with expectation. Additionally the background grey colored peptides show many false positives before adjustment. After adjustment the false positives were decreased considerably. \label{fig:spike_volcano_msstats}}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=.85\textwidth]{sim_new/spike_in_limma_volcano}
\caption{When modeling the experiment with the limma method, the spike in peptides again follow the expected log fold change better after adjusting for changes in protein level. However, while the fold change is much more accurate, the majority of spike in peptides do not have a significant adjusted pvalue. In this case, the known differencial peptides are missed by the model. In terms of false positives, the results are very similar to the proposed method, with many false positives before adjustment and much fewer after. \label{fig:spike_volcano_limma}}
\end{figure}
...
\centering
 ...
 \centering
	...
	\caption{The overlap of differencial modified peptides between the PTM model with and without global protein level adjustment.In this experiment, many more PTMs became insignificant after adjustment then became significant. In this case this is mainly due to the large lack of overlap between the modified peptides and unmodified proteins, stemming from not having a global profiling run.}
 \end{subfigure}\vspace{-5mm}
 \begin{subfigure}{\textwidth}
 \centering
	\includegraphics[height=.5\textwidth]{sim_new/usp30_venn_diagramm_matching_only}
	\caption{The same overlap as above, but looking only at modified peptides with a matching unmodified protein so adjustment can be performed. In this case we see significantly less peptides become insignificant after adjustment. Again we can check if peptides are becoming insignificant due to an increase in variance. To check this we look at modified peptides whose adjusted fold change is within 10\% of their unadjusted fold change and they become insignificant after adjustment. In this experiment there are only 25 peptides that meet this criteria, a very small percentage of all modified peptides.}
 \end{subfigure}
...
\todo{Devon: Update this section per Olga's commentsv}

The proposed approach allows us to calculate the sample size needed to achieve a desired statistical power as described in Section \ref{sec:design}. As described in Section \ref{sec:test} the proposed method adjusts for the underlying protein abundance in the PTM significance analysis, which corrects the confounding factor with a cost of increased variation. When the variation is increased, the experiment requires a larger number of replicates to reach the same power. Thus the statistical power is dependent on the variance from both the modified peptide and unmodifed protein as well as the number of replicates in both the modified and unmodified runs.

We compared the statistical power in experiments with different numbers of replicates, variance, and fold change for both the modified and unmodified runs. The results can be seen in \sfigref{fig:power_sd_combo}
