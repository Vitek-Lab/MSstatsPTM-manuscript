...
\]
Alternatively, run-level summary to be described can also be used. The difference between the means of PTM abundance in Conditions $i$ and $i'$ are estimated as
\[
...
Table \ref{tab:dataDescription} summarizes the datasets used in this manuscript and their experimental designs. There were 6 total experiments, 3 for which ground truth was known and 3 biological investigations. For the ground truth experiments, 2 were generated via computer simulation with differing parameters and the other was an experiment with heavy labeled spike-in peptides. The biological investigations represent a variety of experimental designs and acquisition methods. Two were acquired via Tandem Mass Tag (TMT) labeling, while the other was a label free experiment. Of the two TMT experiments, one was analyzed as a time series and the other a group comparison. 

Details of raw data, R scripts with MSstatsPTM analyses, and result of statistical analyses for the datasets are available in either MassIVE.quant or Github (with direct links available in the table).
...
\hline
%%%% colnames of table
&  & \multicolumn{4}{c|}{Experimental design} & Data availability \\
& Dataset & \multirow{2}{*}{Comparison} & No. of & No. of Bio.& \multirow{2}{*}{Details in}   & (MassIVE.quant  \\
&  & & Conditions & Replicates &  & or R package) \\
\hline
\hline
 &&&&&& \\[-0.05in]
%%%% DDA:ControlledMix
\multirow{5}{*}{\rotatebox[origin=c]{90}{Known}}  \multirow{5}{*}{\rotatebox[origin=c]{90}{Ground}} \multirow{5}{*}{\rotatebox[origin=c]{90}{Truth}} & Computer simulation 1 - Label-free & Group & 2/3/4 & 2/3/5/10 & Suppl. Sec. \ref{sec:dataset1} & \href{https://github.com/devonjkohler/MSstatsPTM_simulations}{Github}\\
 &&&&&& \\%[-0.05in]
%\hline 
%&&&&&&&& \\[-0.05in]
% \cline{2-9}
% &&&&&&&& \\[-0.05in]
%%% DDA iPRG2015
& Computer simulation 2 - missing and low features & Group & 2/3/4 & 2/3/5/10 & Suppl. Sec. \ref{sec:dataset2} & \href{https://github.com/devonjkohler/MSstatsPTM_simulations}{Github} \\
 &&&&&& \\%[0.05in]
%\hline 
% &&&&&& \\[-0.05in]
%%% DDA Cox2014
& SpikeIn benchmark & Group & 4 & 2 & Suppl. Sec. \ref{sec:benchmark} & \\
 &&&&&& \\%[-0.05in]
\hline
\multicolumn{7}{c}{ } \\ [0.02in]
\hline 
 &&&&&& \\%[-0.05in]
%%%%%%%%%%%%%%%%%%
%%%% biological study
%%%%%%%%%%%%%%%%%%
%% DDA Meierhofer
\multirow{3}{*}{\rotatebox[origin=c]{90}{Biological}} \multirow{3}{*}{\rotatebox[origin=c]{90}{Investigation}} & Human - Ubiquitination - TMT & Group & 6 & 2 & Suppl. Sec. \ref{sec:ipah} &  \\
&&&&&& \\%[-0.05in]
% \cline{2-9}
% &&&&&&& \\[-0.05in]
%%%% SRM Picotti 2009
& Mouse - Phosphorylation - TMT & Time course & 6 & 2 & Suppl. Sec. \ref{sec:shigella} & \href{https://massive.ucsd.edu/ProteoSAFe/dataset.jsp?task=4878d777c6b34cf8aaf8477e93140c4d}{MSV000085565} \\
&&&&&& \\%[-0.05in]
% \cline{2-9}
% &&&&&&& \\[-0.05in]
%%%% SRM Surinova 2015, training
& Human - Ubiquitination - Label-free & Group & 4 & 2 & Suppl. Sec. \ref{sec:usp30} &\\ [0.02in]
 &&&&&& \\
 \hline
\end{tabular}\\
\end{tiny}
\caption{ \small {\bf Experimental datasets in this manuscript} 
```Dataset'' is the code name of the dataset used in this manuscript.
`Comparison'' is the type of experimental design (group comparison, or time course).
``No. of Conditions'' states the number of conditions in the dataset.
``No. of Bio. Replicates'' states the number of biological replicates per condition.
Details regarding the experimental design for each dataset are available in the section listed in the ``Details in'' column.
``Data availability'' column shows the MassIVE.quant ID or location on github. 
}
...
\centering
	...
	\caption{}
\caption{Recall results of Simulation 2. The advantage of using the proposed approach is apparent when looking at simulation 2, which includes limited observations and the presence of missing values. In the case of recall the proposed method performs stronger than Limma and t-test in nearly every model. Even at lower replicates the proposed method still outperformed Limma. Again the lowest performing method was $t$-test.}
...
\centering
 ...
 \centering
	...
	\caption{Comparing the global profiling of protein $KI67\_MOUSE|E9PVX6$ with the modification at site $T215$. In this case the modification and global protein appear to show very small to no differences between conditions, however after adjusting for change in global protein abundance, the modification is statistically significant. Additionally, this profile plot shows the large difference in available features between modified peptides and global proteins.}
 \end{subfigure}
 \begin{subfigure}{\textwidth}
 \centering
	\includegraphics[width=.9\textwidth]{sim_new/Difference_Shigella_Volcano}
	\caption{The volcano plot of the WT\_Uninfect and WT\_Early comparison show the specifics of the adjustment. The modification looks to be flat, with a log fold change of $.185$, while the global profiling showed a small negative fold change of $-.619$. While both exhibit small changes, when combined we can see a log fold change of $.804$ and adjusted p-value of $.0156$. In this case we can see the strength of the proposed method, showing noteworthy modifications which would be missed otherwise.}
 \end{subfigure}
...
\caption{Comparing the global profiling of protein $P52209$ with the modification of the protein at site $K059$. The modification appears generally unchanged between all conditions, whereas the global profiling run shows the CCCP and Combo conditions have a higher relative abundance compared to the Control and USP30\_OE. This indicates that the modification actually had an effect when comparing CCCP and Combo to Control and USP30\_OE. However it is not entirely clear as one unmodified peptide feature appears to be changed, while the other does not. This uncertainty is another result of not running a separate global profiling run. With a global profiling run, many unmodified features are generally quantified, removing the uncertainty that comes with low feature counts.}
...
\centering
 ...
 \centering
	...
	\caption{The power of an experiment targeting PTMs with the same variance, .15, for the modified and unmodified peptides. Predictably when the replicates are high for both modified and unmodified peptides the power is much higher. Conversely at low replicates for each the power is much lower. The interesting part of this chart is when the replicates are different between runs. With equal variance, it does not matter if the PTM replicates or protein replicates are higher.}
