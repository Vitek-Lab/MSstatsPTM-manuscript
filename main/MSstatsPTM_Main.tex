...
For one protein, the PTM site is represented by $K$ spectral features (peptide ions, distinguished by their cleavage residues and charge states). 
The number of modified and unmodified features can vary across proteins. Some log-intensities may be outliers, and some spectral features can be missing. 
...
\begin{eqnarray}
H_{0}: \Delta = \mu_{i}^{\ast} - \mu_{i^{\prime}}^{\ast} = 0 \ \text{vs} \ 
H_{a}: \Delta = \mu_{i}^{\ast} - \mu_{i^{\prime}}^{\ast} \neq 0
\label{eq:conv_null_hyp}
\end{eqnarray}
Unfortunately, the population parameter is inherently confounded with the overall changes in protein abundance. To account for this, it is advantageous to change the null hypothesis to
\begin{eqnarray}
...
\label{eq:null_hyp}
\end{eqnarray}
where $\mu_i$ and $\mu_{i^{\prime}}$ reflect the overall protein abundances in Condition $i$ and Condition $i^{\prime}$. These quantities can be estimated using protein features without the modification site.
...
\begin{eqnarray}
y^*_{ijk} = \hat{\mu}^{*}_{i} + \epsilon^{\ast}_{ij}, \ \epsilon^{\ast}_{ij} \mathop\sim^{iid} N(0, \sigma^{*2}),\ i=1,\ldots,I,\ j=1,\ldots,J,\ k=1,\ldots,K
...
\medskip \noindent 
The basic $t$-test can be extended to account for the confounding of changes in PTM abundance and overall changes in protein abundance\todo{OV: not sure where to put this ref ~\cite{Schwammle2015}- could you check it belongs here?} ~\cite{THOMAS2020,Mertins:2013}. Typically this is done by first summarizing the feature intensities in each run, separately for modified and unmodified features, e.g. \todo{with the sum on the original scale}, dividing the summary of the PTM abundance by the summary of the protein abundance,  \todo{and then taking a log?}. Specifically, for condition $i$ and biological replicate $j$, the approach calculates
\begin{eqnarray}
...
\begin{eqnarray}
u_{ij} = (\mu^{\ast}_{i}-\mu_i) + \epsilon^{\prime}_{ij}, \text{where } \epsilon^{\prime}_{ij} \mathop\sim^{iid} N(0, \sigma^{\prime 2}), i \in \{1, 2\}
\label{eq:ttest_model}
\end{eqnarray}
...
$MSstats$~\cite{Choi:2014} is designed for protein significance analysis and statistical relative quantification of proteins and peptides in global, targeted and data-independent proteomics. The package takes as input a list of log$_2$-intensities $y_{ijk}$, used to characterize the identified protein. For each protein, the feature log-intensities are first summarized into a single value per protein per run in the subplot model. This is done using Tukey's median polish (TMP)~\cite{Tukey:1977}, a simple and robust procedure, which iteratively fits a two-way additive model with the effects of run and feature, summarizing them into a single value per run. These summarized values are then used as input to fit a linear model based on the family of linear mixed-effects models \cite{Bolker2009} \cite{Faraway:2006}. For example, the model for a TMT experiment would be as follows 
\todo{OV: this is very different from how we usually present MSstats. Check either the MSstatsTMT paper or the draft of Ting's most recent paper (or maybe her dissertation?) Could you try using the same presentation?}
\begin{eqnarray}
& u_{ijm} = \beta_0 + c_{i}\beta_1 + + r_j\beta_2 + mix_m\beta_3 + \epsilon^{\ast}_{ij}, \text{where } \epsilon^{\ast}_{ijm} \mathop\sim^{iid} N(0, \sigma^2), & \\ 
& mix_m \mathop\sim^{iid} N(0, \sigma^2_m), r_j \mathop\sim^{iid} N(0, \sigma^2_j), \sum_{i=0}^{I} c_i = 0 &
\end{eqnarray}

The model enables the estimation of $\hat{\Delta}$ and the standard error. These methods are implemented in the R packages $MSstats$ and $MSstatsTMT$~\cite{Huang:2020}.

$MSstats$ has a number advantages over simpler methods. First, the approach extends to many types of complex experimental design, including those with multiple sources of variation, such as the TMT model above, and unbalanced designs. Run-level summarization has been shown to be more robust compared to directly using individual features as input to the statistical model~\cite{Choi:2014}. Additionally, missing value imputation can be performed before run-level summarizing using an accelerated failure time (AFT) model~\cite{Wei:1992}. Finally, the methods include optional implementation of empirical bayes moderation, described above, when applied to TMT experiments~\cite{Huang:2020}.
While $MSstats$ is a powerful tool for the quantification of the global protein, it does not have the tools to detect differentially abundant PTMs. It is designed to estimate the difference in a protein between conditions in~\eqref{conv_null_hyp}, and is not designed to remove confounding with the changes in overall protein abundance, and cannot directly test the null hypothesis in~\eqref{null_hyp}.
