...
\todo{I do not think this is right. I believe there should be a summarization first, such that the data are $\hat{y}_{ij}$, the model becomes $\hat{y}^*_{ij} = \mu^{*}_{i} + \epsilon^{\ast}_{ij}, \ \epsilon^{\ast}_{ij} \mathop\sim^{iid} \mathcal{N}(0, \sigma^{*2})$ and the df are $df=I(J-1)$. The index k should drop. Could you start by introducing the summarization? What do you think re keeping or removing the hat on $\hat{y}_{ij}$?}
The basic Analysis of Variance (ANOVA) model~\cite{kutner_etal_04a}  is
\begin{eqnarray}
y^*_{ijk} = \mu^{*}_{i} + \epsilon^{\ast}_{ij}, \ \epsilon^{\ast}_{ij} \mathop\sim^{iid} \mathcal{N}(0, \sigma^{*2}),\ i=1,\ldots,I,\ j=1,\ldots,J,\ k=1,\ldots,K
\label{eq:ttest_model}
\end{eqnarray}
The model allows us to estimate $\hat{\Delta}$ and its standard error, and test the null hypothesis in \eqref{conv_null_hyp} by comparing the model-based test statistic against the Student distribution with $df=I(JK-1)$ \todo{$df=I(J-1)$} degrees of freedom in balanced designs.
...
\end{eqnarray}
The approach then models these values with the basic ANOVA, which corresponds to \todo{if we keep a hat on $\hat{y}_{ij}$, we should probably keep a hat on $\hat{u}_{ij}$}
\begin{eqnarray}
...
The prior distributions are estimated from the data across all the proteins and all the PTM as described in \todo{XXX}. 
With this approach, testing the null hypothesis in~\eqref{null_hyp} amounts to combining the PTM- and protein-specific estimates of variation with a consensus estimate obtained from all the PTM and all the proteins. \todo{It's a bit vague. I am wondering if an equation would help}
...
\todo{I rewrote the model so that it uses $\mu_i$ directly rather than conditions}
\todo{Should we use a hat on top of y, to indicate that we model the summaries?}
\begin{eqnarray}
& y_{ij} = \mu_i + Run_{j} + \epsilon_{ij}, \  \text{where } Run \stackrel{iid}{\sim} \mathcal{N}(0, \sigma^2_j), \epsilon_{ij} \stackrel{iid}{\sim} \mathcal{N}(0, \sigma^2) & 
\label{eq:msstats_model}
\end{eqnarray}
\todo{Is it true that we add run in label-free experiments? I thought it was only with TMT. But maybe I forgot. Could you check? If we don't include run, then give a model for label-free, and then contrast with TMT}
\todo{Is it true that we have a separate variance parameter for each run $ \sigma^2_j$? I don't think so. I think it should be a separate sigma (maybe denote it $\tau^2$), but same for all the runs}
...
\todo{I suggest that you follow the same order as Figure 3 (my suggested version), one paragraph per step. Describe each step, and refer to both the figure, the quantity in the equations and the MSstats functions. Specifically: design; data acquisition; data processing and requirements; converters; steps of statistical analysis. I would state upfront that all the steps before last are performed separately for each data type, and not repeat it in later paragraphs}

The proposed methods are implemented in the open source R package $MSstatsPTM$, overviewed in~\figref{msstatsptm_design}. $MSstatsPTM$ is applicable to most experimental designs, label-free data acquisitions such as DDA, DIA, SRM and label-based data acquisitions such as TMT. $MSstatsPTM$ takes as input lists of identified and quantified spectral features, produced by spectral processing tools such as MaxQuant, Progenesis, or Spectronaut. 

$MSstatsPTM$ requires the processing tool to identify the amino acid in the protein sequence where the modification occurred (i.e. the modification site). This will generally include the amino acid abbreviation, plus it's number in the protein sequence. For example, a modification on a 70th amino acid in the sequence serine should be marked as ``S70''.  

Occasionally the outputs of data processing tools only include the peptide sequence with the modified amino acid highlighted, without indicating where in the protein sequence the modification occurred. For these cases $MSstatsPTM$ includes functionality for identifying this location \todo{what do you mean by 'manually'? The users have to manually input it?} manually, given the modified peptide sequence and a FASTA file with the entire protein sequence. The package includes converters for seamless integration \todo{integration of what?} with multiple such tools. \todo{Maybe put all the discussion of converters in one place - integrate with the first paragraph? } The converters can optionally keep peptides with both single and multiple modification sites, or filter out only those with multiple sites. If multiple sites are kept, these are grouped separately from the singly modified peptide so as not to confound the effect \todo{confound how? The effect of what? We use confounding to indicate the changes in protein abundance. This looks like a different confounding. Could you clarify?}. Additionally, peptides with oxidation can also be removed if the effect is believed to confound the modifications of interest \todo{confound how? The effect of what?}. For experimental designs that include separate runs enriched in modified peptides and global profiling runs, conversion is performed separately for the runs enriched in modified peptides and for the global profiling runs.
...
\caption{\todo{Looks great. Maybe to the left add vertical text 'Upstream experiments and computation' and  'MSstatsPTM' next to the corresponding boxes. No need for a capital L in lysate. 'Group comparison among PTM' and 'Group comparison among unmodified proteins'. I would add () next to the function names, to clarify that these are functions} The $MSstatsPTM$ workflow. The names of the R functions used for each step are highlighted in purple and the output notations are highlighted in red. The package is applicable to label-free data acquisitions such as DDA, DIA, SRM and label-based data acquisitions such as TMT. It takes as input lists of identified and quantified spectral features for the PTM and unmodified protein, produced by spectral processing tools such as MaxQuant, Progenesis, or Spectronaut.  Conversion, summarization and statistical modeling are performed separately for the PTM and for the unmodified proteins. At the end, the model-based summaries are combined to adjust the changes in the PTM abundance for changes in unmodified protein abundance. Data visualizations can optionally be created at each step.}
...
\label{fig:sim2_acc}
\end{subfigure}
\caption{\todo{In the figure titles and in the caption, explain which simulation is 'clean' and which is 'noisy'. Use the same names for the datasets as in the Datasets section throughout. ANOVA is an abbreviation and needs all capital letters - could you check throughout in the figures} Dataset 1 \& 2: Computer simulation. a) All the considered methods in the first computer simulation correctly calibrated FDR when adjusting for changes in protein abundance. In comparison, the methods without accounting for the protein-level changes resulted in off-target, high false positive rates. b) The advantage of using the proposed approach was apparent when including limited observations and missing values. Looking at accuracy, the proposed method outperformed ANOVA and Limma in nearly every model.
}
