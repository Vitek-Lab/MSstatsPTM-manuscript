...
\figref{data-structure} schematically illustrates this data structure for one PTM site, in a special case of a label-free experiment with $I=2$ conditions and $J=2$ biological replicates per condition.
...
\label{eq:null_hyp}
\end{eqnarray}
where $\mu_i$ and $\mu_{i^{\prime}}$ reflect the overall log$_2$ protein abundances in Condition $i$ and Condition $i^{\prime}$. These quantities are estimated using protein features without with and without the modification site.
...
Based on the model, the approach tests the null hypothesis in \eqref{conv_null_hyp}, by comparing the test statistic against the Student distribution with $df=I(JK-1)$ degrees of freedom in balanced designs.
\todo{I do not think this is right. I believe there should be a summarization first, such that the data are $\hat{y}_{ij}$, the model becomes $\hat{y}^*_{ij} = \mu^{*}_{i} + \epsilon^{\ast}_{ij}, \ \epsilon^{\ast}_{ij} \mathop\sim^{iid} \mathcal{N}(0, \sigma^{*2})$ and the df are $df=I(J-1)$. The index k should drop. Could you introduce the summarization first? What do you think re keeping or removing the hat on $\hat{y}_{ij}$?}
...
\end{eqnarray}
The approach then models these values as \todo{if we keep a hat on $\hat{y}_{ij}$, we should probably keep a hat on $\hat{u}_{ij}$}
\begin{eqnarray}
...
\medskip \noindent The two versions of ANOVA above can be expanded with Empirical Bayes moderation in Limma~\cite{ Ritchie_15a, Schwammle2015, Smyth:2004, Smyth:2005, Zhu,Chappell:2021}. A typical application of Limma takes as input $u_{ij}$, and for each PTM fits the linear model in~\eqref{ttest_model}. 
The method differs from the ratio-based ANOVA in specifying additional prior distributions for the model parameters.
The prior distributions are estimated from the data across all the proteins and all the PTM as described in \todo{XXX}. 
With this approach, testing the null hypothesis in~\eqref{null_hyp} amounts to combining the PTM-specific and the consensus variance. \todo{I am wondering if an equation would help}
In  experiments with few biological replicates, the resulting standard errors are often smaller, and the degrees of freedom are often larger than in absence of the moderation~\cite{Ritchie_15a}. 
Thus the approach tends to increase the sensitivity of detection of differential abundance.

Since Limma only improves upon the methods above in estimation of variation, it has the same limitations in terms of the experimental designs. The method is only directly applicable to experiments with at most two variance components, and cannot account for all the sources of variation in more complex experiments. Additionally, it requires the same experimental design for the modified and unmodified features so that the ratio calculation can be performed. Finally, there is no self contained implementation of the methods to PTMs, requiring manual transformation and application by the user.
...
\end{eqnarray}
\todo{Is it true that we have a separate variance parameter for each run $ \sigma^2_j$? I don't think so. I think it should be a separate sigma, but same for all the runs}
...
\end{eqnarray}
\todo{There is no $\mu$ in Eq 6. Could you explain how $\mu$ relates to $Condition$? Alternatively, we can re-write the ANOVA models so that we use zero sum parametrization rather than the cell means parametriation}
