...
\todo{DK: I think we should just delete this part entirely. It isn't how we compared the method and is not really used in the literature from what I can tell.}
...
\medskip \noindent The two versions of $t$-test \todo{OV: ANOVA?} above can be expanded with Empirical Bayes moderation in Limma~\cite{ Ritchie_15a, Schwammle2015, Smyth:2005, Zhu,Chappell:2021}. In particular, the ratio-based version models the ratios $\hat{u}_{ij}$ in~\eqref{adj_estimation}. Limma then fits a linear model using the transformed inputs.
...
\begin{eqnarray}
& Y_{mtcb} = \mu + \text{Mixture}_m + \text{TechRep(Mixture)}_{t(m)} + \text{Condition}_c + Subject_{mcb} + \epsilon_{mtcb}, & \\ 
& \text{where }  \text{Mixture}_m  \mathop\sim^{iid} N(0, \sigma^2_M),  \text{TechRep(Mixture)} \mathop\sim^{iid} N(0, \sigma^2_T), & \\
& \sum_{c=1}^{C} \text{Condition}_c = 0, \epsilon_{mtcb} \mathop\sim^{iid} N(0, \sigma^2) &
\end{eqnarray}
