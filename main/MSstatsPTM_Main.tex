...
We compared the performance of $MSstatsPTM$ to that of $Limma$ and $ANOVA$. Since $Isobar$-$PTM$ is only applicable to experiments with TMT labeling, it could not be applied to the datasets with known ground truth in this manuscript, and was excluded from the comparisons. All methods were compared both before and after adjusting for changes in the unmodified protein. $MSstatsPTM$ was compared to to base $MSstats$, modeled as seen in \eqref{msstats_model} on the peptide level. $ANOVA$ without adjustment was modeled as shown in \eqref{ttest_model}, and will be referred to as $basic$ $ANOVA$. $ANOVA$ with adjustment is referred to as $ANOVA$ and was modeled as in \eqref{ttest_ratio_model}. Finally, $Limma$ without adjustment uses the same model formula in \eqref{ttest_model}, while including a moderated variance estimation, and is referred to as $basic$ $Limma$. $Limma$ with adjustment is referred to as $Limma$ and was modeled as in \eqref{ttest_ratio_model} including moderated variance estimation. $MSstatsPTM$ was used without imputing missing values, and without Empirical Bayes moderation. All the evaluations were done at the FDR-adjusted p-value cutoff of $q=.05$. More details are in {\bf Supplementary Sec. \ref{supp-sec:experiments}}. 
